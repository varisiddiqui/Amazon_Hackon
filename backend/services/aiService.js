...
import { getStudyRecommendation } from "../data/timetableData.js";
...
import { askGemini, summarizeWithGemini, isGeminiConfigured, getGeminiConfig } from "./geminiService.js";
...
export async function summarizeNoticeText(text) {
  if (isGeminiConfigured() && text) {
    try {
      const geminiSummary = await summarizeWithGemini(text);
      if (geminiSummary) return geminiSummary;
    } catch (err) {
      console.warn("Gemini notice summary fallback:", err.message);
    }
  }
  ...
  return {
    ...
    meta: { poweredBy: "CampusGPT (local)", provider: "local" },
  };
}
...
export async function askAI(question, user) {
  ...
  if (isGeminiConfigured()) {
    try {
      return await askGemini(question, user);
    } catch (err) {
      console.warn("Gemini API fallback to CampusGPT rules:", err.message);
    }
  }

  // Fallback: rule-based CampusGPT
  ...
  return {
    ...
    ...response,
    meta: {
      poweredBy: "CampusGPT (local)",
      provider: "local-rules",
      dataSources: ["timetable", "assignments", "attendance", "events", "notices"],
    },
  };
}

export { getGeminiConfig, isGeminiConfigured };
